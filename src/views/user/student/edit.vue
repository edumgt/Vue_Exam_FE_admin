    ...
    <q-card flat bordered>
      <q-card-section>
        <q-form @submit="saveUser" class="q-gutter-md">
          ...
          <q-select v-model="form.userLevel" :options="levelOptions" label="학년" outlined dense emit-value map-options />
...
export default {
  ...
  data () {
    return {
      form: { userName: '', password: '', realName: '', age: null, sex: null, birthDate: '', phone: '', userLevel: null, status: 1, role: 1 },
      isEdit: false
    }
  },
  computed: {
    levelOptions () { return this.enumItemStore.user.levelEnum.map(e => ({ label: e.value, value: e.key })) },
  ...
  methods: {
    ...
    goBack () { this.$router.push({ path: '/user/student/list' }) }
  }
}
</script>
